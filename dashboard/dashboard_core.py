...
def plugin_data_dir() -> Path:
    ...
    path = home / "plugin-data" / "mnemosyne-native-dashboard"
...
class DashboardStore:
    ...
    def _enrich_memory(d: dict[str, Any], memory_kind: str) -> dict[str, Any]:
        ...
        d["degradation_weight"] = (
            DEGRADATION_WEIGHTS.get(degradation_tier or 1, 1.0) if memory_kind == "episodic" else 1.0
        )
    ...
    def realtime_status(self) -> dict[str, Any]:
        ...
        if deltasync_supported and delta_sync_cls is not None:
            deltasync_methods = [
                name
                for name in ("sync_to", "sync_from", "compute_delta", "apply_delta", "get_checkpoint", "set_checkpoint")
        ...
        realtime_generation = (
            "mnemosyne-3.x"
            if mnemosyne_version.startswith("3.")
            else "mnemosyne-2.6"
    ...
    def _realtime_event_rows(self, limit: int = 25, include_inactive: bool = False) -> list[dict[str, Any]]:
        ...
        with self.connect() as con:
            ...
            for table, memory_kind in (("working_memory", "working"), ("episodic_memory", "episodic")):
                ...
                timestamp_expr = (
                    "COALESCE(timestamp, created_at, '')" if "created_at" in columns else "COALESCE(timestamp, '')"
                )
                ...
                where = (
                    "1=1" if include_inactive else "(valid_until IS NULL OR valid_until > ?) AND superseded_by IS NULL"
                )
    ...
    def realtime_event_delta(
        self, seen_ids: set[str] | list[str] | tuple[str, ...] | dict[str, str], limit: int = 25
    ) -> list[dict[str, Any]]:
    ...
    def stats(self) -> dict[str, Any]:
        with self.connect() as con:
            ...
            for table, memory_kind in [("working_memory", "working"), ("episodic_memory", "episodic")]:
                ...
                columns = self._columns(con, table)
                by_source_raw += [
                    dict(r, tier=memory_kind, memory_kind=memory_kind)
                ...
                for row in con.execute(
                    f"SELECT {veracity_expr} AS veracity, COUNT(*) AS count FROM {table} GROUP BY veracity"
                ):
                ...
                if table == "episodic_memory":
                    ...
                    for row in con.execute(
                        f"SELECT {tier_expr} AS degradation_tier, COUNT(*) AS count FROM episodic_memory GROUP BY degradation_tier"
                    ):
                        degradation_counts[_degradation_label(row["degradation_tier"])] += int(row["count"] or 0)
                    if "degraded_at" in columns:
                        degraded_count = int(
                            con.execute(
                                "SELECT COUNT(*) FROM episodic_memory WHERE COALESCE(degraded_at, '') != ''"
                            ).fetchone()[0]
                        )
                    if "tier" in columns:
                        due_tier2 = int(
                            con.execute(
                                "SELECT COUNT(*) FROM episodic_memory WHERE COALESCE(tier, 1) = 1 AND COALESCE(created_at, timestamp, '') < ?",
                                (tier2_ts,),
                            ).fetchone()[0]
                        )
                        due_tier3 = int(
                            con.execute(
                                "SELECT COUNT(*) FROM episodic_memory WHERE COALESCE(tier, 1) = 2 AND COALESCE(created_at, timestamp, '') < ?",
                                (tier3_ts,),
                            ).fetchone()[0]
                        )
            ...
            def aggregate(rows: list[dict[str, Any]], key: str, limit: int = 20) -> list[dict[str, Any]]:
                ...
                return [
                    {key: k, "count": v} for k, v in sorted(totals.items(), key=lambda kv: kv[1], reverse=True)[:limit]
                ]
            ...
            by_degradation = [
                {
                    ...
                    "count": int(degradation_counts.get(label, 0)),
                    "weight": DEGRADATION_WEIGHTS[tier],
                }
    ...
    def list_memories(
        ...
        with self.connect() as con:
            ...
            for table, memory_kind in wanted:
                ...
                if q:
                    where.append(
                        "(content REGEXP ? OR id REGEXP ? OR session_id REGEXP ? OR source REGEXP ? OR scope REGEXP ?)"
                    )
                ...
                if memory_kind == "episodic":
                    ...
                    if due:
                        where.append(
                            f"(({tier_expr} = 1 AND COALESCE(created_at, timestamp, '') < ?) OR ({tier_expr} = 2 AND COALESCE(created_at, timestamp, '') < ?))"
                        )
        ...
        rows.sort(
            key=lambda r: (
                ...
                if sort == "importance"
                else int(r.get("recall_count") or 0)
                if sort == "recall"
                else (r.get("timestamp") or r.get("created_at") or "")
            ),
            reverse=sort != "oldest",
        )
    ...
    def lifecycle_dashboard(self, limit: int = 50) -> dict[str, Any]:
        ...
        recently_degraded = self.list_memories(
            kind="episodic", status="active", degraded_only=True, sort="recent", limit=limit
        )
        high_importance_degraded = [m for m in recently_degraded if float(m.get("importance") or 0) > 0.5][:limit]
        stats = self.stats()
        degradation_counts = {
            row["degradation_label"]: int(row["count"] or 0) for row in stats.get("by_degradation", [])
        }
        counts = {
            "hot": degradation_counts.get("hot", len(hot)),
            "warm": degradation_counts.get("warm", len(warm)),
            "cold": degradation_counts.get("cold", len(cold)),
            "due_for_degradation": int(stats.get("degradation", {}).get("due_tier2") or 0)
            + int(stats.get("degradation", {}).get("due_tier3") or 0),
        ...
        queues = {
            "hot": {
                "title": "Hot memories",
                "description": "Tier 1 episodic memories retain full detail and full lifecycle weight.",
                "filter": {"kind": "episodic", "degradation_tier": "1", "sort": "oldest"},
                "items": hot,
            },
            "warm": {
                "title": "Warm memories",
                "description": "Tier 2 episodic memories are compressed and carry reduced recall weight.",
                "filter": {"kind": "episodic", "degradation_tier": "2", "sort": "oldest"},
                "items": warm,
            },
            "cold": {
                "title": "Cold memories",
                "description": "Tier 3 episodic memories keep key signal only and carry the lowest lifecycle weight.",
                "filter": {"kind": "episodic", "degradation_tier": "3", "sort": "oldest"},
                "items": cold,
            },
            "due_for_degradation": {
                "title": "Due for degradation",
                "description": "Hot or warm episodic memories old enough to move to the next lifecycle tier.",
                "filter": {"kind": "episodic", "due_for_degradation": "1", "sort": "oldest"},
                "items": due,
            },
            "recently_degraded": {
                "title": "Recently degraded",
                "description": "Memories already compressed into a lower lifecycle tier.",
                "filter": {"kind": "episodic", "degraded_only": "1", "sort": "recent"},
                "items": recently_degraded,
            },
            "high_importance_degraded": {
                "title": "High-importance degraded",
                "description": "Important degraded memories worth checking before further lifecycle compression.",
                "filter": {"kind": "episodic", "degraded_only": "1", "sort": "importance"},
                "items": high_importance_degraded,
            },
        }
        cards = [
        ...
        return {
            ...
            "thresholds": {
                ...
                "weights": {str(k): v for k, v in DEGRADATION_WEIGHTS.items()},
            },
            "counts": counts,
            "cards": cards,
            "queues": queues,
    ...
    def session_detail(self, session_id: str, limit: int = 200) -> dict[str, Any]:
            ...
            events.append(
                {
                    ...
                    "title": m.get("tier") or "memory",
                    "preview": str(m.get("content") or "")[:240],
                    "item": m,
                }
            )
        for t in triples:
            events.append(
                {
                    "type": "triple",
                    "timestamp": t.get("created_at") or t.get("valid_from") or "",
                    "title": t.get("predicate") or "triple",
                    "preview": f"{t.get('subject')} → {t.get('object')}",
                    "item": t,
                }
            )
                ...
                events.append(
                    {
                        ...
                        "title": f"{c.get('items_consolidated')} items",
                        "preview": c.get("summary_preview") or "",
                        "item": c,
                    }
                )
        events.sort(key=lambda e: e.get("timestamp") or "", reverse=True)
        return {
            ...
            "counts": {
                ...
                "triples": len(triples),
                "consolidations": len([c for c in consolidations if c.get("session_id") == session_id]),
                "events": len(events),
            },
            "memories": memories,
            "triples": triples,
        ...
        with self.connect_rw() as con:
            ...
            for table in ("working_memory", "episodic_memory"):
                if table in self._tables(con):
                    cur = con.execute(
                        f"UPDATE {table} SET valid_until = ?, superseded_by = NULL WHERE id = ?", (now, memory_id)
                    )
    ...
    def supersede_memory(
        self, memory_id: str, content: str, importance: float | None = None, backup: bool = True
    ) -> dict[str, Any]:
        memory_id = (memory_id or "").strip()
        ...
        metadata.update({"supersedes": memory_id, "created_by": "mnemosyne-native-dashboard"})
        backup_info = self.backup_database() if backup else None
        with self.connect_rw() as con:
            tables = self._tables(con)
            ...
            for table in ("working_memory", "episodic_memory"):
                if table in tables:
                    con.execute(
                        f"UPDATE {table} SET valid_until = ?, superseded_by = ? WHERE id = ?",
                        (now, replacement_id, memory_id),
                    )
                ...
                if {"id", "content"} <= cols:
                    ...
                    for k, v in optional.items():
                        if k in cols:
                            ...
                            values.append(v)
                    con.execute(
                        f"INSERT OR REPLACE INTO memories ({', '.join(keys)}) VALUES ({', '.join(['?'] * len(keys))})",
                        values,
                    )
    ...
    def _category_for_text(text: str) -> str:
        ...
        if not scores:
            return "Other"
        if scores.get("Health / wearables") and re.search(
            r"\b(health check|healthy state|system health|failing health)\b", hay
        ):
            scores["Health / wearables"] -= 8
        if scores.get("Agent memory") and any(
            term in hay for term in ("dashboard", "labyrinth", "memory palace", "visualiser", "visualizer", "viewport")
        ):
    ...
    def today_digest(self, day: str = "", limit: int = 80) -> dict[str, Any]:
        ...
        degradation_counts = Counter(
            str(m.get("degradation_label") or "not degraded")
            for m in memories_today
            if m.get("memory_kind") == "episodic"
        )
        ...
        top_entities = [
            {"label": x, "count": n} for x, n in Counter(self._entity_terms(*entity_values, limit=80)).most_common(16)
        ]
        return {
            "day": day,
            "read_only": True,
            ...
            "breakdowns": {
                ...
                "veracity": [
                    {"label": k, "count": v, "weight": VERACITY_WEIGHTS.get(k, VERACITY_WEIGHTS["unknown"])}
                    for k, v in veracity_counts.most_common()
                ],
    ...
    def _context_type(text: str, category: str, kind: str = "memory") -> tuple[str, str]:
        ...
        if any(
            term in hay
            for term in ("prefers", "preference", "wants", "expects", "avoid", "should not", "do not", "tone", "style")
        ):
            return "Preference", "confirmed"
        if any(
            term in hay
            for term in ("project", "plugin", "dashboard", "github", "release", "migration", "server", "config")
        ):
    ...
    def inferred_profile(self, limit_per_section: int = 10) -> dict[str, Any]:
        ...
        def add_row(
            *, kind: str, label: str, item: dict[str, Any], importance: object, timestamp: object, category: str
        ) -> None:
        ...
        for t in self.triples(limit=500):
            ...
            add_row(
                ...
                label=text,
                item=t,
                importance=t.get("confidence"),
                timestamp=t.get("created_at") or t.get("valid_from"),
                category=category,
            )
            ...
            return [
                {
                    ...
                    "query": "" if k == "Unclassified" else k,
                }
    ...
    def constellation(self, limit: int = 240) -> dict[str, Any]:
        ...
        def touch(
            label: str, kind: str = "entity", weight: float = 1.0, category: str = "Other", timestamp: str = ""
        ) -> dict[str, Any]:
            label = str(label or "unknown").strip()[:80]
            if not label:
                label = "unknown"
            node = nodes_by_label.setdefault(
                label,
                {
                    "id": f"n{len(nodes_by_label) + 1}",
                    "label": label,
                    "kind": kind,
                    "category": category,
                    "weight": 0.0,
        ...
        for m in memories:
            ...
            category = self._category_for_text(content)
            m_node = touch(
                f"memory:{str(m.get('id') or '')[:10]}",
                kind="memory",
                weight=float(m.get("importance") or 0.4) * 1.5,
                category=category,
                timestamp=ts,
            )
            ...
            for entity in self._entity_terms(content, limit=3):
                ...
                edges.append(
                    {
                        "id": f"em{m.get('id')}-{e_node['id']}",
                        "source": m_node["id"],
                        "target": e_node["id"],
                        "label": "mentions",
                        "kind": "memory",
                        "item": {"memory_id": m.get("id"), "entity": entity},
                    }
                )

        nodes = sorted(
            nodes_by_label.values(),
            key=lambda n: (float(n.get("weight") or 0), str(n.get("last_seen") or "")),
            reverse=True,
        )[:limit]
        ...
        return {
            ...
            "nodes": nodes,
            "edges": edges,
            "clusters": [{"label": k, "count": v} for k, v in clusters.most_common()],
        }
    ...
    def recall_debug(self, q: str = "", limit: int = 20) -> dict[str, Any]:
        q = (q or "").strip()
        rows = (
            self.list_memories(kind="all", q=q, sort="importance", limit=max(1, min(int(limit or 20), 100)))
            if q
            else []
        )
        ...
        for row in rows:
            ...
            approx_score = round(
                ((0.55 * term_score) + (0.30 * importance) + (0.15 * min(recall_count, 10) / 10))
                * float(row.get("effective_memory_weight") or 1.0),
                4,
            )
            reasons = []
            if matched_terms:
            ...
            if row.get("degradation_label"):
                reasons.append(
                    f"Lifecycle: {row.get('degradation_label')} ×{float(row.get('degradation_weight') or 1):.2f}"
                )
            ...
            items.append(
                {"memory": row, "approx_score": approx_score, "matched_terms": matched_terms, "reasons": reasons}
            )
        ...
        with self.connect() as con:
            ...
            for tbl in ["memoria_facts", "memoria_timelines", "memoria_instructions", "memoria_preferences"]:
                if tbl in tables and "session_id" in self._columns(con, tbl):
                    for row in con.execute(
                        f"SELECT session_id, count(*) AS c FROM {tbl} GROUP BY session_id ORDER BY c DESC LIMIT 10"
                    ):
